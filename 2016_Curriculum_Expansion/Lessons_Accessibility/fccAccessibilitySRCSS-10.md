...
1) `display: none;` or `visibility: hidden;` hides content for everyone, including screen reader users
2) Zero values for pixel sizes, such as `width: 0px; height: 0px;` removes that element from the flow of your document, meaning screen readers will ignore it
...
assert(code.test(/width:\s*?1px/gi), 'message: Change the <code>width</code> of the <code>sr-only</code> class to <code>1px</code>.');

assert(code.test(/height:\s*?1px/gi), 'message: Change the <code>height</code> of the <code>sr-only</code> class to <code>1px</code>.');
```
